...
class WebStories {
    ...
    async webStoriesList(req,res) {
        try {
            ...
            page = parseInt(page);
            console.log('story_category_id:',story_category_id);
    ...
    async login(req, res) {
        try {
            ...
            return res.status(200).json({ status:200, message: 'LoggedIn successfully', token })
        } catch (error) {
            console.log('Error:', error)
            return res.status(500).json({ status:500, message: 'Internal server error' })
        }
    ...
    async createStory (req, res) {
        try {
            ...
            await StoryModel.insertMany(slideList)
    ...
    async userStory(req,res) {
        try {
            const userId = req.userId
            const userSlides = await StoryModel.find({user_id: userId})
            return res.status(200).json({ status:200, message: 'User story fetch successfully ', data: userSlides })
        } catch (error) {
            console.log('Error:', error)
         return res.status(500).json({ status:500, message: 'Internal server error' })
        }
    ...
    async editStory(req, res) {
        try {
            ...
            const body =  req.body
            if(!body.heading || !body.description || !body.image || !body.story_category_id || !body.story_id) {
                return res.status(403).json({ status: 403, message: 'Invalid request data'})
            }
            const storyDetail = await StoryModel.findOne({_id: new ObjectId(storyId)})
            if (!storyDetail) {
                return res.status(404).json({ status:404, message: 'Story not found' })
            }
            storyDetail.heading = body.heading 
            storyDetail.description = body.description 
            storyDetail.image = body.image 
            storyDetail.story_category_id = body.story_category_id 
    ...
    async getBookmarkStory(req,res) {
        try {
            ...
            const match = {
                _id: userData.bookmark_story_ids,
            }
